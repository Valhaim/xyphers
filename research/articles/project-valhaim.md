...
Valhaim is live in early alpha at [valhaim.com](https://valhaim.com). This article describes the larger economy it is being developed toward and the thermodynamic research supporting that ambition.
...
The [Core Thesis](core-thesis.md) introduces future entropy, **S<sub>τ</sub>**, through a traveller crossing bridges between islands. The same calculation can examine how widely a network's possible destinations spread after a declared number of steps. The small **τ** names that future horizon.
...
THAIM is the accounting unit and positive expansion receipt. At a shared, independently grounded temperature **α**, an action receives **THAIM<sub>+</sub> = α · max(0, ΔS<sub>τ</sub>)**. Opening the declared future earns a receipt; narrowing it earns zero.
...
The [Valhaim kernel evidence record](../physics/valhaim-thermodynamic-kernel/README.md) contains its frozen protocol, sealed runner, exact results, and claim boundaries. Its README identifies the original execution revision and artifact hashes.

The [Filter proof and boundary record](../evidence/local-future-test/README.txt) supplies the local-to-global theorem and its later counterexamples. The [research index](../README.md) connects these studies to the broader Xypher programme.

[Explore Valhaim · Early alpha](https://valhaim.com) · [Return to the Core Thesis](core-thesis.md)
